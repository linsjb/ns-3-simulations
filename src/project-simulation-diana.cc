...
static void GenerateTraffic (Ptr<Socket> socket, Ptr<ExponentialRandomVariable> randomSize,	Ptr<ExponentialRandomVariable> randomTime)
{
 ...
 //  tch.AddInternalQueues (handle, 3, "ns3::DropTailQueue", "MaxSize", StringValue (queueSize+"p"));
  ...
  sourceD->Connect (remote);
   ...
   Ptr<ExponentialRandomVariable> randomTimeC = CreateObject<ExponentialRandomVariable> ();
   randomTimeC->SetAttribute ("Mean", DoubleValue (meanC));

   double meanD = 0.001; //0.5 ms
   Ptr<ExponentialRandomVariable> randomTimeD = CreateObject<ExponentialRandomVariable> ();
   ...
   Simulator::ScheduleWithContext (sourceC->GetNode ()->GetId (), Seconds (2.0), &GenerateTraffic, sourceC, randomSize, randomTimeC);
   Simulator::ScheduleWithContext (sourceD->GetNode ()->GetId (), Seconds (2.0), &GenerateTraffic, sourceD, randomSize, randomTimeD);
