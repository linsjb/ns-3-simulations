...
std::vector<double> poisson(std::vector<double> U, double lambda) {

    std::vector<double> fy;
    std::vector<double> fye(U.size(), 0);
    ...
    for (int i = 0; i < N; i++) {
        ...
        fy.push_back(pow(lambda,i) * exp(-lambda) * (1/factorial));
    }
...
int main (int argc, char *argv[]){
    ...
    int a = 1, c = 1, m = 1, seed = 1, n = 1;
    bool lcg = false, ns3 = false, all = false, poi = false, rvn = false, pdf = false, write_to_file = true;
    ...
    File_writer fw;
    // fw.set_filename("m" + std::to_string(m) + "-c" + std::to_string(c) + "-a" + std::to_string(a));
    fw.set_filename("poi-m" + std::to_string(m) + "-c" + std::to_string(c) + "-a" + std::to_string(a));
    ...
    if(poi)
        fw.append(poisson(lcg_rand(a, c, m, seed, n), lambda));
